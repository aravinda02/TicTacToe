...
/** This was generated by chatGPT. A terminal representation of each entry within the Q-Table
 */
void AI::printQtable(std::vector<std::tuple<std::array<char, 9>, int, int>> qTable){
    std::cout << "Q-Table for X Player:\n";
    for (const auto& entry : qTable) {
        const auto& [state, action, reward] = entry;  
        std::cout << "State: ";
        for (char c : state) {
            std::cout << c << " ";
        }
