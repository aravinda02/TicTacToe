...
int main(){
    ...
    /* lines 57 to 79 were generated by ChatGPT*/
    ...
    for (const auto& entry : qTables.first) {
    ...
    for (const auto& entry : qTables.second) {
        const auto& [state, action, reward] = entry;  // Structured binding
        std::cout << "State: ";
        for (char c : state) {
            std::cout << c << " ";
        }
